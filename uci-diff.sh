...
print_usage() {
	...
	echo -e "actions:\n"
	echo -e "\t-d|--diff file1 file2 out-file         //create a JSON diff between file1 and file2 and print the result in out-file"
...
extract_config(){
	if [ "$1" == "all" ]; then
		...
		for file in $(ls /etc/config); do
			echo -e "$(ubus call uci get "{\"config\":\"$file\"}" | sed "s/\"values\":/\"$file\":/g" | sed 's/^\t}$/\t},/g' | sed '/^[{}]$/d')" >>$2
		done
		echo -e '}' >>$2
	else
